import {
  ...
  GuessedWord,
  WordSearchState,
} from "../src/wordsearch/play";
import { WordPosition, WordSearch } from "../src/wordsearch/play/types";
import { wordGridReducer } from "../src/wordsearch/play/wordGridReducer";
import { GridCellPosition } from "../src/wordsearch/common-types";
import { getState, getWord } from "../src/wordsearch/play/getState";
  ...
  describe("clicked letter - reducer", () => {
    describe("no selected letter", () => {
      ...
      it("should set the selected cell to selected", () => {
        ...
        const expectedCell: GuessedCell = {
          isGuessed: false,
          isSelected: true,
          letter: "A",
        };
      ...
      it("should not mutate the cell or the row", () => {
        const selectedCell: GuessedCell = {
          isGuessed: false,
          isSelected: false,
          letter: "A",
        };
      ...
      it("should leave other rows unchanged", () => {
        const notSelectedCellRow: GuessedCell[] = [
          { isGuessed: false, isSelected: false, letter: "B" },
        ];
        const state: WordSearchState = {
          firstSelectedCell: undefined,
          wordGrid: [
            [{ isGuessed: false, isSelected: false, letter: "A" }],
            notSelectedCellRow,
          ],
          guessedWords: [],
          canTemplate: false,
      ...
      describe("matching word", () => {
        it("should set the word to guessed", () => {
          ...
          const state: WordSearchState = {
            ...
            wordGrid: [
              [
                { isGuessed: false, isSelected: true, letter: "A" },
                { isGuessed: false, isSelected: false, letter: "B" },
              ],
            ],
        ...
        describe("cell guessed", () => {
          ...
          describe("horizontal", () => {
            it("should set the cells to guessed - left to right", () => {
              ...
              row.forEach((cell, colIndex) => {
                if (
                  colIndex >= leftToRightPosition.start.col &&
                  colIndex <= leftToRightPosition.end.col
                ) {
            ...
            it("should set the cells to guessed - right to left", () => {
              ...
              row.forEach((cell, colIndex) => {
                if (
                  colIndex <= rightToLeftPosition.start.col &&
                  colIndex >= rightToLeftPosition.end.col
                ) {
                  expect(cell.isGuessed).toBe(true);
                } else {
                  expect(cell.isGuessed).toBe(false);
          ...
          const arrangeAndTest = (
            wordPosition: WordPosition,
            inRangePredicate: (row: number, col: number) => boolean
          ) => {
            const { newState } = arrange(wordPosition);
            newState.wordGrid.forEach((row, rowIndex) => {
              row.forEach((cell, colIndex) => {
                expect(cell.isGuessed).toBe(
                  inRangePredicate(rowIndex, colIndex)
                );
              });
            });
          };
          describe("vertical", () => {
            it("should set the cells to guessed - top to bottom", () => {
              ...
              arrangeAndTest(
                topToBottomPosition,
                (rowIndex, colIndex) =>
                  colIndex === topToBottomPosition.start.col &&
                  rowIndex >= topToBottomPosition.start.row &&
                  rowIndex <= topToBottomPosition.end.row
              );
            });
            ...
            it("should set the cells to guessed - bottom to top", () => {
              ...
              arrangeAndTest(
                bottomToTopPosition,
                (rowIndex, colIndex) =>
                  colIndex === bottomToTopPosition.start.col &&
                  rowIndex <= bottomToTopPosition.start.row &&
                  rowIndex >= bottomToTopPosition.end.row
              );
            });
          });

          describe("diagonal", () => {
            const arrangeAndTestDiagonals = (
              wordPosition: WordPosition,
              diagonals: GridCellPosition[]
            ) => {
              diagonals = diagonals.concat(
                wordPosition.start,
                wordPosition.end
              );
              arrangeAndTest(wordPosition, (rowIndex, colIndex) => {
                return diagonals.some(
                  (diagonal) =>
                    diagonal.row === rowIndex && diagonal.col === colIndex
                );
              });
            };
            it("should set the cells to guessed - left/right bottom/top", () => {
              ...
              const diagonals: GridCellPosition[] = [{ row: 6, col: 1 }];
            ...
            it("should set the cells to guessed - left/right top/bottom", () => {
              ...
              const diagonals: GridCellPosition[] = [{ row: 6, col: 4 }];
            ...
            it("should set the cells to guessed - right/left bottom/top", () => {
              ...
              const diagonals: GridCellPosition[] = [{ row: 6, col: 6 }];
