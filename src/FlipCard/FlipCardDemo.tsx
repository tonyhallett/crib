...
import { Button, Checkbox, FormControlLabel, Input, MenuItem, Select } from "@mui/material";
...
class SingleValueAnimation implements IAnimationWithParameters{
  description:string
  parameters:AnimationParameters[]
  constructor(private filter:string,private filterSuffix = "",private initialAmount:number = 0){
...
class NoParametersAnimation implements IAnimationWithParameters{
  constructor(public description:string,private animationSequence:FlipCardProps["animationSequence"]){
...
const animationsWithParameters:IAnimationWithParameters[] = [
  ...
  new NoParametersAnimation("flip move flip",[flipAnimation, movementSegment, flipAnimation]),
...
function cloneParameters(parameters:AnimationParameters[]):StateAnimationParameters[]{
...
export function FlipCardDemoX(props:{top:boolean}) {
  const [selectedAnimationWithParametersIndex,setSelectedAnimationWithParametersIndex] = useState(0);
  const [animationParameters,setAnimationParameters] = useState(cloneParameters(animationsWithParameters[0].parameters));
  const [redSuit,setRedSuit] = useState(true);
  const [pictureCard,setPictureCard] = useState(true);
  const [animationSequence,setAnimationSequence] = useState<FlipCardProps["animationSequence"]|undefined>(undefined);
  ...
  const playingCard = {pips:pictureCard ? Pips.King: Pips.Ace,suit:redSuit?Suit.Hearts:Suit.Spades};
  return (
    <>
      <div style={{backgroundColor:"white",position:"absolute",top:props.top? 0:undefined, bottom:props.top? undefined:0}}>
        <FormControlLabel control={<Checkbox checked={redSuit} onChange={redSuitChanged} />}  label="Red Suit ?"/>
        <FormControlLabel control={<Checkbox checked={pictureCard} onChange={pictureCardChanged} />}  label="Picture card ?"/>
        <Select value={selectedAnimationWithParametersIndex} onChange={(event) => {
          setSelectedAnimationWithParametersIndex(event.target.value as number);
          const parameters = animationsWithParameters[event.target.value as number].parameters;
          setAnimationParameters(cloneParameters(parameters))
          //
        }}>
          {animationsWithParameters.map((animationWithParameters,i) => {
            return <MenuItem key={i} value={i} >{animationWithParameters.description}</MenuItem>
          })}
        </Select>
      ...
      <div>
        {
          animationParameters.map((parameter,i) => {
            return <div key={`${animationsWithParameters[selectedAnimationWithParametersIndex].description}_${i}`}>
            <div>{parameter.description}</div>
            <Input  type="number" key={i} value={parameter.amount} onChange={(event) => {
              const newParameters = animationParameters.map((parameter,j) => {
                if(j === i){
                  const amount = event.target.value === "" ? "" : Number(event.target.value);
                  if(amount !== ""){
                    animationsWithParameters[selectedAnimationWithParametersIndex].parameters[i].amount = amount;
                  }
      ...
      <Button
          onClick={() => {
            setAnimationSequence(animationsWithParameters[selectedAnimationWithParametersIndex].getAnimationSequence());
          }}
        >
          Animate
      </Button>
