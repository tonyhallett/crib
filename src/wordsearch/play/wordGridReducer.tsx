...
function findWord(
  ...
  return guessedWords.find(
    (word) =>
      word.start.row === wordStart.row &&
      word.start.col === wordStart.col &&
      word.end.row === wordEnd.row &&
      word.end.col === wordEnd.col
  );
}
...
export function wordGridReducer(
  state: WordSearchState,
  action: LetterClickAction
) {
  const newState = {
    ...state,
  };
  if (state.firstSelectedCell === undefined) {
    ...
    const newWordGrid = updateSelectedCell(
      state.wordGrid,
      action.gridCellPosition.row,
      action.gridCellPosition.col,
      true
    );
    newState.wordGrid = newWordGrid;
  } else {
    const matchingWord = findWord(
      state.guessedWords,
      state.firstSelectedCell,
      action.gridCellPosition
    );
    ...
    if (matchingWord) {
      ...
      const newWordGrid = updateSelectedCell(
        state.wordGrid,
        state.firstSelectedCell.row,
        state.firstSelectedCell.col,
        false
      );
      newState.wordGrid = newWordGrid;
    }
