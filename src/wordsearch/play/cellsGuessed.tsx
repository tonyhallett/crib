...
const diagonalCellsGuessed = (
  ...
  const cellOnDiagonal = (col: number, row: number) => {
    return diagonals.some(
      (diagonal) => diagonal.row === row && diagonal.col === col
    );
  };
  return wordGrid.map((row, rowIndex) => {
    if (rowInRange(rowIndex)) {
