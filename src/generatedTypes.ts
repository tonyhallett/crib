...
export const hubFactory = {
    ...
    crib(connection:signalR.HubConnection){
        return {
           ...
           sendFriendRequests:(friends:string[]) => connection.send('SendFriendRequests', friends),
           acceptFriendRequest:(friendship:Friendship) => connection.send('AcceptFriendRequest', friendship),
           createMatch:(matchOptions:MatchOptions) => connection.send('CreateMatch', matchOptions),
           discard:(matchId:string, discard1:PlayingCard, discard2?:PlayingCard) => connection.send('Discard', matchId, discard1, discard2),
           peg:(matchId:string, peggedCard:PlayingCard) => connection.send('Peg', matchId, peggedCard),
