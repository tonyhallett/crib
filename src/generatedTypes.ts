...
export interface CribClient {
  ...
  ready(matchId: string, playerId: string, myMatch: MyMatch): void;
  peg(
    matchId: string,
    playerId: string,
  ...
  crib(
    ...
    const untypedClient: UntypedClient<CribClient> = {
      discard(
        ...
        return client.discard(matchId, playerId, cutCard, myMatch);
      },

      ready(matchId: string, playerId: string, myMatch: MyMatch) {
        return client.ready(matchId, playerId, myMatch);
      },
      ...
      peg(
        ...
        return client.peg(matchId, playerId, peggedCard, myMatch);
      },
