...
const fm: {
  createVisualElement(element: HTMLElement | SVGElement): void;
...
export function StaggerCenterTest() {
  return (
    <MultiElementComparison
      sequence={createStaggerSequence(stagger(1, { from: "center" }))}
    />
  );
}
...
export function StaggerFirstTest() {
  return (
    <MultiElementComparison
      sequence={createStaggerSequence(stagger(1, { from: "first" }))}
    />
  );
}
...
export function StaggerLastTest() {
  return (
    <MultiElementComparison
      sequence={createStaggerSequence(stagger(1, { from: "last" }))}
    />
  );
}
...
export function MultiSegmentTransitionEndComparison() {
  return (
    <MultiElementComparison sequence={multiSegmentTransitionEndSequence} />
  );
}
...
export function MultiSegmentTransitionSequenceDelayComparison() {
  return (
    <MultiElementComparison
      sequence={multiSegmentTransitionEndSequence}
      sequenceOptions={{ delay: 3 }}
    />
  );
}
...
export function MotionValueComparison() {
  ...
  return (
    <MultiElementComparison
      sequenceOptions={{ delay: 5, duration: 10 }}
      motionValueAndkey={{ key: "x", value: motionValue }}
      sequence={[
    ...
    <MultiElementComparison
      sequence={[
        [
          "div",
          { x: 200 },
          { type: "spring", stiffness: 1000, damping: 100, duration: 3 },
        ],
      ]}
    />
  );
...
function MultipleElementDemo(props: {
  ...
  const elements = new Array(props.numElements).fill(0).map((_, index) => {
    ...
    if (props.motionValueAndKey) {
      ...
      return (
        <motion.div key={index} style={motionStyle}>
          {oldOrNew}
        </motion.div>
      );
    } else {
  ...
  return <div ref={props.scope}>{elements}</div>;
}
  ...
  return (
    <>
      ...
      <MultipleElementDemo
        ...
        old={false}
        motionValueAndKey={props.motionValueAndkey}
      />
      <MultipleElementDemo
        scope={scopeOld}
        yStart={yStart + space * (numElements + 1)}
        numElements={numElements}
        old={true}
        motionValueAndKey={props.motionValueAndkey}
      />
...
export function BadTransitionEnd() {
  ...
  const animateClickHandler = useCallback(() => {
    ...
    const visualElement = visualElementStore.get(
      myRef.current as unknown as HTMLElement
    );
...
class FakedAnimationPlaybackControls implements AnimationPlaybackControls {
  constructor(
    private realAnimationPlaybackControls: AnimationPlaybackControls,
    private fakeAnimationPlaybackControls: AnimationPlaybackControls
  ) {}
  ...
  then(
    ...
    onReject?: VoidFunction | undefined
  ): Promise<void> {
...
export function FakeAnimateExample() {
  ...
  const animationPlaybackControlsRef = useRef<
    AnimationPlaybackControls | undefined
  >();
  const fakeAnimationPlaybackControlsRef = useRef<
    AnimationPlaybackControls | undefined
  >();
  ...
  const playClickHandler = useCallback(() => {
    if (animationPlaybackControlsRef.current === undefined) {
      ...
      const fakeGrouping = new fm2.GroupPlaybackControls([
        fakeXAnimation,
        fakeYAnimation,
        fakeXAnimation2,
      ]);
      ...
      const realAnimationPlaybackControls = animate(
        [
          [
            scope.current,
            { x: 600, y: 400 },
            { x: { duration: 10, delay: 1 }, y: { duration: 4 } },
          ],
          [scope.current, { x: 200 }, { duration: 5, delay: 0 }],
        ],
        { repeat: 2, repeatType: "reverse", repeatDelay: 2 }
      );
      animationPlaybackControlsRef.current = new FakedAnimationPlaybackControls(
        realAnimationPlaybackControls,
        fakeGrouping
      );
  ...
  return (
    <>
      <div style={{ overflow: "hidden" }} ref={scope}>
        ***
      </div>
      <div style={{ position: "absolute", bottom: 100 }}>
        ...
        <Slider
          value={time}
          max={durationRef.current === undefined ? 0 : durationRef.current}
          disabled={stopped}
          onChange={(evt, newValue) => {
            newValue = newValue as number;
            setTime(newValue);
            (
              animationPlaybackControlsRef.current as AnimationPlaybackControls
            ).time = newValue;
          }}
        />
        <div>Speed</div>
        <Slider
          value={speed}
          max={durationRef.current === undefined ? 0 : 10}
          disabled={stopped}
          onChange={(evt, newValue) => {
            newValue = newValue as number;
            setSpeed(newValue);
...
export function OldControls() {
  ...
  return (
    <>
      <div style={{ position: "absolute", bottom: 100 }}>
        <Button disabled={isPlaying || stopped} onClick={playClickHandler}>
          Play
        </Button>
        <Button disabled={!isPlaying || stopped} onClick={pauseClickHandler}>
          Pause
        </Button>
        <Button disabled={stopped} onClick={stopClickHandler}>
          Stop
        </Button>
        <Button disabled={!isPlaying || stopped} onClick={completeClickHandler}>
          Complete
        </Button>
