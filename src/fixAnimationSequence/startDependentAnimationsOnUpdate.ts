...
import { AnimationSegmentInfo, ElementAnimationDefinition } from "./createAnimationsFromSegments";
...
export function startDependentAnimationsOnUpdate(
    ...
    const endingTransition = resolvedAnimationDefinition.transition[keyToOnUpdate] as any;
    ...
    const segmentDelay = dependent.startTime - (animationSegmentInfo.startTime + animationSegmentInfo.totalDuration);
    const resolvedDependencies = resolveDependencies([dependent], totalTime, sequenceDuration, segmentDelay, true);
    ...
    endingTransition.onComplete = () => {
        ...
        resolvedDependencies.forEach((defs, key) => {
            defs.forEach((def) => {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const animation = animateElements(key as Element, def.keyframes, { ...def.transition as any, transitionEnd: def.transitionEnd });
