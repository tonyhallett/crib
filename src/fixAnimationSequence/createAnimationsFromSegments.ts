...
export type IntermediateAnimationSegmentInfo = Omit<
  ...
  "elementKeyTotalTime" | "totalDuration"
>;
...
export type NewResolvedAnimationDefinition =
  | ElementAnimationDefinition
  | MotionValueAnimationDefinition;
export type NewResolvedAnimationDefinitions = Map<
  Element | MotionValue,
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    function resolveTime(
      ...
      const calculatedDelay =
        typeof delay === "function" ? delay(elementIndex, numElements) : delay;
    ...
    if (isMotionValue(subject)) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      resolveTime(
        keyframes as ValueKeyframesDefinition,
        transition as any,
        0,
        1,
        "",
...
export function createAnimationsFromSegments(
  ...
  return resolveDependencies(
    ...
    false,
    scope === undefined ? [] : scope.animations
  );
}
