...
export function determineDependencies(
  ...
  animationSegmentInfos.forEach((animationSegmentInfo) => {
    ...
    animationSegmentInfos.forEach((otherAnimationSegmentInfo) => {
      if (
        otherAnimationSegmentInfo !== animationSegmentInfo &&
        /* chain check todo */ isPossibleDependentByStartTime(
          otherAnimationSegmentInfo,
          animationSegmentInfo
        )
      ) {
        const commonKeys = animationSegmentInfo.keys.filter((key) =>
          otherAnimationSegmentInfo.keys.includes(key)
        );
        ...
        if (commonKeys.length > 0) {
          ...
          if (currentDependency !== undefined) {
            const currentEndTime =
              currentDependency.startTime + currentDependency.totalDuration;
            if (
              dependency.startTime + dependency.totalDuration >
              currentEndTime
            ) {
