import { AnimationPlaybackControls, AnimationScope, useUnmountEffect } from "framer-motion"
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export class SegmentScopeInternal<T extends Element = any> implements SegmentScope<T> {
    stopAnimations(){
...
export function useAnimateSegments<T extends Element = any>() {
    const scope: SegmentScopeInternal<T> = useConstant(() => (new SegmentScopeInternal<T>()))
